...
impl CompilerFlags {
    pub fn from(name: &str, value: &Value) -> Result<Self, String> {
        match name {
            "release_target" => match value.as_integer() {
                Some(i) => {
                    if i.is_negative() {
                        return Err(format!(
                            "Illegal Java version {i}, version must be positive"
                        ));
                    }
            ...
            "javadoc_lints" => match value.as_array() {
                Some(array) => {
                    ...
                    if lints.is_empty() {
                        return Err(String::from("At least one lint must be given"));
                    }
            ...
            _ => Err(format!("Unrecognized compiler flag {name}")),
        }
    }
...
mod tests {
    ...
    fn rejects_negative_release_target() {
        ...
        assert!(error.contains("Illegal Java version"));
    }
    ...
    fn rejects_empty_lint_list() {
        ...
        assert_eq!(error, "At least one lint must be given");
    }
    ...
    fn rejects_unknown_compiler_flag() {
        ...
        assert!(error.contains("Unrecognized compiler flag"));
    }
}

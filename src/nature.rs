        ...
        match self {
            Nature::Eclipse => {
                ...
                let _ = write(
                    ...
                    eq_sep_config::generate_config(template::generate_eclipse_config(
                        configuration,
                    )),
                );
                let _ = write(
                    ".settings/org.eclipse.m2e.core.prefs",
                    eq_sep_config::generate_config(template::generate_maven_config()),
                );
                let _ = write(".project", template::generate_project(project).unwrap());

                let _ = write(
                    ".classpath",
                    template::generate_classpath(project, configuration, regexes).unwrap(),
                );
                let _ = write(".project", template::generate_project(project).unwrap());
            }
            Nature::Maven => {
                ...
                let _ = write(
                    "pom.xml",
                    template::generate_pom(project, configuration).unwrap(),
                );
            }
        }
    }

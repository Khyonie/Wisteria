...
use crate::{
    ...
    maven::repository::{self, ArtifactVersion},
    util::{files, toml_utils},
};
        ...
        match toml.get("type") {
            Some(val) if val.is_str() => {
                ...
                match val.as_str().unwrap() {
                    ...
                    _ => Err((
                        format!("Unknown dependency type \"{}\"", val.as_str().unwrap()),
                        31,
                    )),
                }
            }
        ...
        match self {
            Dependency::LocalFile { path, javadoc } => {
                ...
                let canon_path = match pathbuf.canonicalize() {
                    Ok(p) => p,
                    Err(e) => {
                        return Err((format!("Could not canonicalize path \"{path}\": {e}"), 62))
                    }
                };
    ...
    pub fn is_shaded(&self, name: &str, configuration: &Configuration) -> Option<bool> {
        ...
        match self {
            ...
            Dependency::LocalRepository {
                ...
                update_policy,
                javadoc,
            } => Some(shaded.contains(&String::from(name))),
            Dependency::FetchFromUrl {
                url,
                update_policy,
                javadoc,
